...
{
    public class SaveCourseCommandHandler : IRequestHandler<SaveCourseCommand, ResponseResult>
    {
        ...
        public SaveCourseCommandHandler(IOrgUnitOfWork unitOfWork, CSRedisClient redisClient, IConfiguration _config, SmLogUserOperation smLogUserOperation,
            IMediator mediator)
        {
        ...
        public async Task<ResponseResult> Handle(SaveCourseCommand request, CancellationToken cancellationToken)
        {
            ...
            var pointRatio = SmRatio.Parse(_config["AppSettings:sku_money_to_point"]); // 钱与积分比例
            ...
            //}
            if (request.UpdateGoodsInfos?.Count > 0
                && request.UpdateGoodsInfos.Any(g => g.CashbackType == CashbackTypeEnum.Percent.ToInt() && g.CashbackValue > 100))
            {
                return ResponseResult.Failed("分销自购返现比例不能超过100%，请重新填写！");
            }
            if (request.UpdateGoodsInfos?.Count > 0
                && request.UpdateGoodsInfos.Any(g => g.PointCashBackType == CashbackTypeEnum.Percent.ToInt() && g.PointCashBackValue > 100))
            {
                return ResponseResult.Failed("积分比例不能超过100%，请重新填写！");
            }
            // 检测佣金设置是否ok
            if (request.UpdateGoodsInfos?.Count > 0)
            {
                foreach (var g in request.UpdateGoodsInfos)
                {
                    if (CheckSkuYongjinSettingHasError(g, pointRatio) is CustomResponseException ex)
                        throw ex;
                }
            }
            // sku兑换积分
            if (request.UpdateSkuPointExchanges?.Count > 0)
            {
                foreach (var g in request.UpdateSkuPointExchanges)
                {
                    if (g.Show)
                    {
                        if (g.Point != null && g.Point > 0) continue;
                        else throw new CustomResponseException("兑换积分必须大于0");
                    }
                }
            }
            //
...
values(NEWID(), @Courseid, @Title{i}, @Price{i}, @StartTime{i}, @EndTime{i},@Condition{i}, @CashbackType{i}, @CashbackValue{i},@PJCashbackType{i}, @PJCashbackValue{i},@IsBonusRate{i},@HeadFxUserExclusiveValue{i},@HeadFxUserExclusiveType{i},@desc{i}, @time{i}, @opuser, @time{i}, @opuser, 1) ;";
                ...
                dy.Set("courseid", request.Id)
                   ...
                   .Set("IsPointExchange", request.EnablePointExchange)
                   .Set("LimitedBuyNum", request.SpuLimitedBuyNum)
                   ;
...
,[LastOffShelfTime],[count],[IsInvisibleOnline],[IsExplosions],[IsSystemCourse],[VideoCovers],[Videos],[CanNewUserReward],[NewUserExclusive],[LimitedTimeOffer],[SetTop],[BlackList],[IsPointExchange],[LimitedBuyNum])
...
,@IsInvisibleOnline,@IsExplosions,@IsSystemCourse,@VideoCovers,@Videos,@CanNewUserReward,@NewUserExclusive,@LimitedTimeOffer,@SetTop,@BlackList,@IsPointExchange,@LimitedBuyNum)
;";
...
,VideoCovers=@VideoCovers,Videos=@Videos,[CanNewUserReward]=@CanNewUserReward,[NewUserExclusive]=@NewUserExclusive,[LimitedTimeOffer]=@LimitedTimeOffer,[SetTop]=@SetTop,[BlackList]=@BlackList,[IsPointExchange]=@IsPointExchange,[LimitedBuyNum]=@LimitedBuyNum
  where id=@courseid
 ;";
...
values(NEWID(),@Courseid, @CashbackType, @CashbackValue,@PJCashbackType, @PJCashbackValue, @IsBonusRate, @HeadFxUserExclusiveValue, @HeadFxUserExclusiveType, @NolimitType, @NolimitAfterDate, @NolimitAfterBuyInDays, @NewUserRewardType,@NewUserRewardValue, @time, @opuser, @time, @opuser,1,@ReceivingAfterDays)";
                    var drpDy = new DynamicParameters()
                        ...
                        //.Set("IsBonusRate", request.CashbackValue > 0 ? 1 : request.IsBonusRate ? 1 : 0)//自购返现数值大于0，则工资系数计算自动勾选
                        .Set("IsBonusRate", request.UpdateGoodsInfos.Any(g => g.CashbackValue > 0) ? 1 : request.IsBonusRate ? 1 : 0)
                        //
                    ...
                    if (request.UpdateGoodsInfos?.Any() == true)
                    {
                        TrySave_SkuYongjing_and_Point(request);
                    }

                    // 更新sku兑换积分
                    if (request.UpdateSkuPointExchanges?.Any() == true)
                    {
                        TrySave_SkuPointExchanges(request);
                    }
                    ...
                    // add user log
                    {
                        _smLogUserOperation.SetUserId(request.UserId)
                            .SetClass(nameof(SaveCourseCommand))
                            .SetMethod(request.IsAdd ? "add" : "update")
                            .SetParams("_", request)
                            .SetParams("courseid", request.Id)
                            .SetParams("sku_money_to_point", pointRatio.ToString())                            
        ...
        /// sku佣金与赠送积分
        /// </summary>
        void TrySave_SkuYongjing_and_Point(SaveCourseCommand request)
        {
            ...
            for (var i = 0; i < request.UpdateGoodsInfos.Count; i++)
            {
                var g = request.UpdateGoodsInfos[i];
                sql.AppendLine($@"
update CourseGoodDrpInfo set IsValid=0,ModifyDateTime=@now,Modifier=@opuser where IsValid=1 and GoodId=@Sku{i}_Id ;

insert CourseGoodDrpInfo(Id,courseid,GoodId,CashbackType,CashbackValue,CreateTime,Creator,ModifyDateTime,Modifier,IsValid)
    values(newid(),@courseid,@Sku{i}_Id,@Sku{i}_CashbackType,@Sku{i}_CashbackValue,@now,@opuser,@now,@opuser,1) ;

update CourseGoodPointCashBack set IsValid=0,ModifyDateTime=@now,Modifier=@opuser where IsValid=1 and GoodId=@Sku{i}_Id ;

insert CourseGoodPointCashBack(Id,courseid,GoodId,PointCashBackType,PointCashBackValue,CreateTime,Creator,ModifyDateTime,Modifier,IsValid)
    values(newid(),@courseid,@Sku{i}_Id,@Sku{i}_PointCashBackType,@Sku{i}_PointCashBackValue,@now,@opuser,@now,@opuser,1) ;
                ");
                dys.Set($"Sku{i}_Id", g.GoodsId);
                dys.Set($"Sku{i}_CashbackType", g.CashbackType ?? 0);
                dys.Set($"Sku{i}_CashbackValue", g.CashbackValue ?? 0);
                dys.Set($"Sku{i}_PointCashBackType", g.PointCashBackType ?? 0);
                dys.Set($"Sku{i}_PointCashBackValue", g.PointCashBackValue ?? 0);
            }

            dys.Set("now", DateTime.Now)
                .Set("courseid", request.Id)
                .Set("opuser", request.UserId);

            _orgUnitOfWork.BeginTransaction();
            _orgUnitOfWork.DbConnection.Execute(sql.ToString(), dys, _orgUnitOfWork.DbTransaction);
            _orgUnitOfWork.CommitChanges();
        }
        ...
        /// 更新sku兑换积分
        /// </summary>
        void TrySave_SkuPointExchanges(SaveCourseCommand request)
        {
            var dys = new DynamicParameters();
            var sql = new StringBuilder();

            for (var i = 0; i < request.UpdateSkuPointExchanges.Count; i++)
            {
                var g = request.UpdateSkuPointExchanges[i];
                sql.AppendLine($@"
update CourseGoodsExchange set IsValid=0,ModifyDateTime=@now,Modifier=@opuser where IsValid=1 and GoodId=@Sku{i}_GoodsId ;

insert CourseGoodsExchange(Id,courseid,GoodId,Point,Price,Show,CreateTime,Creator,ModifyDateTime,Modifier,IsValid)
    values(newid(),@courseid,@Sku{i}_GoodsId,@Sku{i}_Point,@Sku{i}_Price,@Sku{i}_Show,@now,@opuser,@now,@opuser,1) ;
                ");
                dys.Set($"Sku{i}_GoodsId", g.GoodsId);
                dys.Set($"Sku{i}_Point", g.Point);
                dys.Set($"Sku{i}_Price", g.Price);
                dys.Set($"Sku{i}_Show", g.Show);
            }
        ...
        /// <summary>检测佣金设置是否ok</summary>
        /// <param name="g"></param>
        /// <param name="ratio">元 : 积分</param>
        /// <returns></returns>
        static CustomResponseException CheckSkuYongjinSettingHasError(UpdateGoodsInfo g, SmRatio ratio)
        {
            var profit = g.Price - (g.Costprice ?? 0m); // 利润
            var yj = 0m;

            if (g?.CashbackValue != null && !Enum.IsDefined(typeof(CashbackTypeEnum), g?.CashbackType))
            {
                return new CustomResponseException("直推佣金单位错误");
            }
            if (g?.PointCashBackValue != null && !Enum.IsDefined(typeof(CashbackTypeEnum), g?.PointCashBackType))
            {
                return new CustomResponseException("积分单位错误");
            }

            var cashbackType = (CashbackTypeEnum)(g?.CashbackType ?? 0);
            var cashbackValue = (g?.CashbackValue ?? 0);
            var point_cashbackType = (CashbackTypeEnum)(g?.PointCashBackType ?? 0);
            var point_cashbackValue = (g?.PointCashBackValue ?? 0);

            if (cashbackType == CashbackTypeEnum.Percent && point_cashbackType == CashbackTypeEnum.Percent)
            {
                if (cashbackValue + point_cashbackValue > 100)
                {
                    if (cashbackValue <= 0 && point_cashbackValue > 0) return new CustomResponseException("请输入正确的赠送积分配置");
                    if (cashbackValue > 0 && point_cashbackValue <= 0) return new CustomResponseException("请输入正确的推广佣金配置");
                }
                else return null;
            }
            else
            {
                yj += cashbackType == CashbackTypeEnum.Percent ? profit * cashbackValue / 100m
                    : cashbackType == CashbackTypeEnum.Yuan ? cashbackValue
                    : 0m;

                yj += point_cashbackType == CashbackTypeEnum.Percent ? profit * point_cashbackValue / 100m
                    : point_cashbackType == CashbackTypeEnum.Yuan ? ratio.RightToLeftValue(point_cashbackValue)
                    : 0m;

                if (yj == 0 || yj <= profit) return null;
                // err
                if (cashbackValue <= 0 && point_cashbackValue > 0) return new CustomResponseException("请输入正确的赠送积分配置");
                if (cashbackValue > 0 && point_cashbackValue <= 0) return new CustomResponseException("请输入正确的推广佣金配置");
            }

            return new CustomResponseException("请输入正确的推广佣金或赠送积分配置");
        }
