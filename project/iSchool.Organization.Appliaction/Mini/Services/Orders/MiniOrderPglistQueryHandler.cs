...
{
    public class MiniOrderPglistQueryHandler : IRequestHandler<MiniOrderPglistQuery, MiniOrderPglistQryResult>
    {
        ...
        public MiniOrderPglistQueryHandler(IOrgUnitOfWork unitOfWork, IMediator mediator, CSRedisClient redis,
        ...
        public async Task<MiniOrderPglistQryResult> Handle(MiniOrderPglistQuery query, CancellationToken cancellation)
        {
            ...
            //待支付 跟 待收货要将部分发货  查询出来
            var where = $@" 1=1 and IsValid=1 AND type>={OrderType.BuyCourseByWx.ToInt()}  
            {
                (query.Status == 0 ? "" :
               query.Status == OrderStatusV2.Unpaid.ToInt() ? "and status=@Status  and datediff(second,CreateTime,getdate())<60*30" :
               query.Status == OrderStatusV2.Shipping.ToInt() ? "and (status=@Status or status=" + (int)OrderStatusV2.PartialShipped + " or status=" + (int)OrderStatusV2.ExWarehouse + ")" :
               query.Status == OrderStatusV2.Paid.ToInt() ? "and (status=@Status or status=" + (int)OrderStatusV2.PartialShipped + ")" :
               "and status=@Status")}
            { "and userid=@UserId".If(query.UserId != null)}
            { $"and (status<200 or status>300)".If(query.Status != (int)OrderStatusV2.RefundOk)}
            ";
            var sql = $@"
