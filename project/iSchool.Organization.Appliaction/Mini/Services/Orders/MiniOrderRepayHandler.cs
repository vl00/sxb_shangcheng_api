...
{
    public class MiniOrderRepayHandler : IRequestHandler<MiniOrderRepayCmd, Res2Result<CourseWxCreateOrderCmdResult_v4>>
    {
        ...
        public MiniOrderRepayHandler(IOrgUnitOfWork orgUnitOfWork, IMediator mediator, CSRedisClient redis,
...
";
            ...
            // 有可能新小程序拿不到当前userid
            if (!cmd.IsNewMpPay && orders.Any(_ => _.Userid != me.UserId))
            {
                throw new CustomResponseException("订单用户信息有误.");
            }

            var userId = orders[0].Userid; //me.UserId;

            // 判断用户是否重复下单
            await using var lck0 = await _lck1fay.LockAsync($"org:lck2:wx_repay_course:" + (cmd.IsNewMpPay ? $"userid_{userId}&" : "") + $"orderid_{cmd.OrderId}", 5 * 1000, 1);
            if (!lck0.IsAvailable)
            {
            ...
            var is_already_payedok = false; // 订单是否其实已支付,可能之前回调处理不成功
            {
                var args_callpay = new WxPayRequest
                {
                    AddPayOrderRequest = new ApiWxAddPayOrderRequest
                    {
                        ...
                        OrderByProducts = products.SelectMany(product => OrderHelper.OrderDetailSpread2ApiOrderByProducts(product, result.OrderId!.Value))
                        ...
                        IsRepay = cmd.IsNewMpPay ? default : 1,
                        IsWechatMiniProgram = cmd.IsWechatMiniProgram,
                        AppId = cmd.AppId,
                        NoNeedPay = (totalpayment == 0 ? 1 : 0),

                        // 用于测试 wx支付2min超时
                        //OrderExpireTime = (orders[0].CreateTime ?? DateTime.Now).AddMinutes(2),
                        // 正式30min
                        OrderExpireTime = (orders[0].CreateTime ?? DateTime.Now).AddMinutes(30),
                ...
                {
                    var msg = GetLogMsg(args_callpay).SetUserId(userId).SetClass(this.GetType().Name);
                    ...
                    {
                        if (ex.Message.IndexOf("请使用原支付的微信账号继续支付") > -1)
                            throw new CustomResponseException(ex.Message, Consts.Err.CallPaidApiError);
