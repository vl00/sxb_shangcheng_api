...
namespace iSchool.Organization.Appliaction.Service.Lables
{
    ...
    /// 【长Id集合】评测卡片业务
    /// </summary>
    public class EvaltsLableByIdsQueryHandler : IRequestHandler<EvaltsLableByIdsQuery, ResponseResult>
    {
        OrgUnitOfWork _orgUnitOfWork;

        public EvaltsLableByIdsQueryHandler(IOrgUnitOfWork unitOfWork)
        {
            this._orgUnitOfWork = (OrgUnitOfWork)unitOfWork; 
        }
        
        public async Task<ResponseResult> Handle(EvaltsLableByIdsQuery request, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                where += $"  and id in ('{string.Join("','", request.Ids)}')  ";
            }
