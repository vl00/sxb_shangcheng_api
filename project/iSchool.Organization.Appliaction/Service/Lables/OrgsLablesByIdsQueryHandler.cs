...
namespace iSchool.Organization.Appliaction.Service.Lables
{
    ...
    /// 【长ID集合】机构卡片列表
    /// </summary>
    public class OrgsLablesByIdsQueryHandler : IRequestHandler<OrgsLablesByIdsQuery, ResponseResult>
    {
        OrgUnitOfWork unitOfWork;

        public OrgsLablesByIdsQueryHandler(IOrgUnitOfWork unitOfWork)
        {
        ...
        public async Task<ResponseResult> Handle(OrgsLablesByIdsQuery request, CancellationToken cancellationToken)
        {
            #region Where
            ...
            {               
                sqlWhere += $"  and o.id in ('{string.Join("','",request.LongIds)}') ";
            }
