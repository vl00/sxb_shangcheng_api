...
{
    public class SendCouponReceiveSuccessTipsCommnadHandler : IRequestHandler<SendCouponReceiveSuccessTipsCommnad>
    {
        ...
        public SendCouponReceiveSuccessTipsCommnadHandler(ITemplateMessageService templateMessageService
            , IMediator mediator
        ...
        public async Task<Unit> Handle(SendCouponReceiveSuccessTipsCommnad request, CancellationToken cancellationToken)
        {
            try
            {
...
#endif
                string expiretxt = request.CouponExpireTime == null ? "该券永久有效！ " : $"该券将于{request.CouponExpireTime.Value.ToString("yyyy-MM-dd HH:mm")}过期，请及时使用！";
                sendRequest.SetData(
                new TemplateDataFiled()
                {
                    Filed = "first",
                    Value = $"恭喜您已成功获得 {request.CouponValue}优惠券！{expiretxt}",
                },
                new TemplateDataFiled()
                {
                    Filed = "keyword1",
