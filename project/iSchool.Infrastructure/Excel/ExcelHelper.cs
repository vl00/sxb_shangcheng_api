...
namespace iSchool.Infrastructure.Excel
{
    /// <summary>
    ...
    public class ExcelHelper : IDisposable
    {
        ...
        public static int DataToExcel(string TargetFileNamePath, IEnumerable<dynamic> sourceData, string sheetName, bool IsWriteColumnName)
        {
            ...
            if (IsWriteColumnName)
            {
                ...
                dynamic columns = sourceData.AsArray()[0];
                string j = JsonConvert.SerializeObject(columns);
                var b = JsonConvert.DeserializeObject<IDictionary<string,string>>(j);
                int colunmNameIndex = 0;
                foreach(string key in b.Keys)
                {
                    ColumnNameRow.CreateCell(colunmNameIndex).SetCellValue(key);
                    colunmNameIndex++;
                }
                WriteRowCount++;
            }
            ...
            for (int row = 0; row < sourceData.Count(); row++)
            {
                ...
                dynamic columns = sourceData.AsArray()[row];
                string j = JsonConvert.SerializeObject(columns);
                var b = JsonConvert.DeserializeObject<IDictionary<string, string>>(j);
                int column = 0;
                foreach(string val in b.Values)
                {
                    newRow.CreateCell(column).SetCellValue(val);
                    column++;
                }
                WriteRowCount++;  //写入下一行
            }
            ...
            //写入到excel中
            FileStream fs = new FileStream(TargetFileNamePath, FileMode.Create, FileAccess.Write);
            workbook.Write(fs);
            //fs.Flush();
            fs.Close();
